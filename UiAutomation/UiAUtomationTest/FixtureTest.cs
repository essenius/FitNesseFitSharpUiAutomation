...
public class FixtureTest
{
    ...
    public void FixtureMsInfo32CheckSetValueResizeMove()
    {
        try
        {
            ...
            Assert.IsTrue(_fixture.ResizeWindowTo(desiredSize), "Resize OK");
            Assert.AreEqual(desiredSize, _fixture.WindowSize, "Size OK after resize");
            ...
            Assert.IsTrue(_fixture.MoveWindowTo(desiredLocation), "Move OK");
            Assert.AreEqual(desiredLocation, _fixture.WindowTopLeft, "Location OK");

            Assert.IsTrue(_fixture.ClickControl("Close"), "Click Close OK");
            UiAutomationFixture.WaitUntilProcessEnds($"ProcessId: {_fixture.ApplicationProcessId}");
