...
public class CalcTest
{
    ...
    [TestMethod, TestCategory("Calc")]
    public void CalcFirstTextUnderMainWindow()
    {
        var window = _fixture.GetControl("ControlType:MainWindow");
        Assert.AreEqual("Calculator", window.FirstTextUnder(), "The first text control under the window contains Calculator");
    }

    [TestMethod, TestCategory("Calc")]
    public void CalcFirstTextUnderResult()
    {
        _fixture.PressKeys("456");
        Assert.AreEqual(
            "456",
            _fixture.FirstTextUnder(Fields.Result), "The first text control under the Result box contains 456"
        );
        Assert.AreEqual("456", _fixture.ValueOfControl(Fields.Result));
    }

    [TestMethod, TestCategory("Calc")]
    public void CalcFirstTextNone()
    {
        var pane = _fixture.GetControl("ClassName:ApplicationFrameInputSinkWindow");
        Assert.IsNull(pane.FirstTextUnder(), "There is no text control under the pane");
    }
