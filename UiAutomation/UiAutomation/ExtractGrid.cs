...
public class ExtractGrid
{
    ...
    public Collection<object> Query()
    {
        ...
        if (control?.AutomationElement?.GetCurrentPattern(UIA_PatternIds.UIA_GridPatternId) is not IUIAutomationGridPattern gridPattern)
        {
            return null;
        }

        var headerCollection = new Collection<string>();
        var headerContainer = new Control("ControlType:Header") { SearchType = SearchType.Deep, Parent = control };
